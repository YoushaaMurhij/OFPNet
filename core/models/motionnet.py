...
class STPN(nn.Module):
    def __init__(self, height_feat_size=3):
    ...
    def forward(self, x):
        ...
        # print(F.interpolate(x_5, scale_factor=(2, 2)).shape)
...
class MotionNet(nn.Module):
    def __init__(self, out_seq_len=20, motion_category_num=2, height_feat_size=3):
        ...
        self.observed_head = sepHead(ch_in=77, ch_out=8)
        self.occluded_head = sepHead(ch_in=77, ch_out=8)
        self.flow_dx_head  = sepHead(ch_in=176, ch_out=8)
        self.flow_dy_head  = sepHead(ch_in=176, ch_out=8)
    ...
    def forward(self, input):
        ...
        cell_class_pred = cell_class_pred.view(Batch, -1, x.size(-2), x.size(-1))
        # Motion State Classification head
        state_class_pred = self.state_classify(x)
        state_class_pred = state_class_pred.view(Batch, -1, x.size(-2), x.size(-1))
        ...
        out3 = self.flow_dx_head(disp)
        out4 = self.flow_dy_head(disp)
...
if __name__ == "__main__":
    model = MotionNet(out_seq_len=8).to("cuda:0")
