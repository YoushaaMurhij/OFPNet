...
class R2AttU_seq(nn.Module):
    def __init__(self,img_ch=23,output_ch=32,t=2, sliced_head=False):
        ...
        self.observed_head = sepHead(ch_in=self.head_in_ch, ch_out=1)
        self.occluded_head = sepHead(ch_in=self.head_in_ch, ch_out=1)
        self.flow_dx_head  = sepHead(ch_in=self.head_in_ch, ch_out=1)
        self.flow_dy_head  = sepHead(ch_in=self.head_in_ch, ch_out=1)
...
def main():
    model = R2AttU_seq(img_ch=23, output_ch=32, t=1).to("cuda:0")
    ...
    # onnx_model = onnx.load("R2AttU_seq.onnx")
    # model_with_shapes = onnx.shape_inference.infer_shapes(onnx_model)
    # onnx.save(model_with_shapes, "R2AttU_seq_with_shapes.onnx")
